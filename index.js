...
(async () => {
    // Inicialización de usuarios con contraseña predefinida
    users = [
        {
            username: "admin",
    ...
    app.post('/api/auth/login', async (req, res) => {
        ...
        if (await verifyPassword(password, user.password)) {
            ...
            return res.status(200).json({ username: user.username, name: user.name, token: user.token });
        } else {
...
(async () => {
    ...
    app.post('/api/auth/login', async (req, res) => {
        ...
        if (await verifyPassword(password, user.password)) {
            ...
            return res.status(401).json({ error: 'Invalid password' });//token invalido
        }
    });

    app.get('/api/reminders', authenticate, (req, res) => {
        const orderedReminders = reminders.sort((a, b) => b.createdAt - a.createdAt);
        res.status(200).json(orderedReminders.map(reminder => ({// ok
            id: reminder.id,
            content: reminder.content,
            important: reminder.important,
            createdAt: reminder.createdAt,
    ...
    app.post('/api/reminders', authenticate, (req, res) => {
        const { content, important } = req.body;
        if (!validateReminder(content, important)) {
            return res.status(400).json({ error: 'Invalid content or important' });//validacion incorrecta
        }
        ...
        reminders.push(newReminder);
        res.status(201).json(newReminder);//ok y crear nuevo recordatorio
    });
    ...
    app.patch('/api/reminders/:id', authenticate, (req, res) => {
        ...
        if (!reminder) return res.status(404).json({ error: 'Reminder not found' });//no existe el recordatorio
