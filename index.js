...
// Simulación de base de datos en memoria (para principiantes)
let reminders = [];
let users = [{ username: 'admin', password: 'password123', token: generateToken() }]; // Contraseña en texto plano (¡INSEGURO!)

function generateToken() {
    return crypto.randomBytes(48).toString('hex');
}

// Middleware de autenticación (versión simplificada)
...
// Ruta de login (versión simplificada)
app.post('/api/auth/login', (req, res) => {
    const { username, password } = req.body;
    const user = users.find(u => u.username === username);
    if (!user) return res.status(401).json({ error: 'User not found' });

    if (user.password === password) { // Comparación de contraseñas en texto plano (¡INSEGURO!)
...
app.get('/api/reminders', authenticate, (req, res) => {
    const orderedReminders = reminders.sort((a, b) => b.createdAt - a.createdAt); // Ordenar por fecha de creación descendente
